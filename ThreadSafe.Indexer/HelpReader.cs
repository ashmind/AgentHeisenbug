...
namespace ThreadSafe.Indexer {
    public class HelpReader {
        ...
        public IEnumerable<TypeDescription> ReadDirectory(DirectoryInfo directory, Func<FileInfo, bool> fileFilter) {
            var files = directory.EnumerateFiles("*.mshc").Where(fileFilter);
